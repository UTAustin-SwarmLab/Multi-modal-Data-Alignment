...
def main(cfg: DictConfig):
    ...
    # Ours
    ax.plot([x[0] for x in ds_roc_points], [x[1] for x in ds_roc_points], 'o-', label=f'Random shuffle (ours). AUC={ds_auc:.3f}', color='blue')
    ax.plot([x[0] for x in class_roc_points], [x[1] for x in class_roc_points], 'o-', label=f'Class level shuffle (ours). AUC={class_auc:.3f}', color='red')
    ax.plot([x[0] for x in obj_roc_points], [x[1] for x in obj_roc_points], 'o-', label=f'Object level shuffle (ours). AUC={obj_auc:.3f}', color='green')
    # CLIP encoders
