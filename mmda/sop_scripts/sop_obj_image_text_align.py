...
def SOP_obj_align(cfg: DictConfig):
    ...
    plot_several_pdf(data_vector_list=[sim_align, sim_unalign], 
                     ...
                     ax=ax)
    if cfg.equal_weights:
        save_fig(fig, cfg.paths.plots_path + f'similarity_score_obj_dim{cfg.sim_dim}_{cfg.train_test_ratio}_noweight.png')
    else:
        save_fig(fig, cfg.paths.plots_path + f'similarity_score_obj_dim{cfg.sim_dim}_{cfg.train_test_ratio}.png')
...
def SOP_CLIP_obj_align(cfg: DictConfig):
    # set random seed
    np.random.seed(cfg.seed)
