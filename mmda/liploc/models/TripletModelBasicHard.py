...
class Model(nn.Module):
    def __init__(self, CFG):
        ...
        self.encoder_camera = ImageEncoder(
            model_name=CFG.trained_image_model_name,
            pretrained=CFG.pretrained,
            trainable=CFG.trainable,
        )
        self.encoder_lidar = ImageEncoder(
            model_name=CFG.trained_image_model_name,
            pretrained=CFG.pretrained,
            trainable=CFG.trainable,
        )
        self.projection_lidar = ProjectionHead(
            embedding_dim=CFG.image_embedding_dim,
            projection_dim=CFG.projection_dim,
            dropout=CFG.dropout,
        )
        self.projection_camera = ProjectionHead(
            embedding_dim=CFG.image_embedding_dim,
            projection_dim=CFG.projection_dim,
            dropout=CFG.dropout,
        )
    ...
    def forward(self, batch):
        ...
        batch_size = batch["camera_image"].shape[
            0
        ]  # This can be different from CFG.batch_size for the last batch
        ...
        camera_embeddings_normalized = (
            camera_embeddings / torch.linalg.norm(camera_embeddings, dim=1)[:, None]
        )
        lidar_embeddings_normalized = (
            lidar_embeddings / torch.linalg.norm(lidar_embeddings, dim=1)[:, None]
        )
        ...
        similarity_matrix = (
            similarity_matrix + torch.eye(batch_size).to(self.device) * 1e6
        )
