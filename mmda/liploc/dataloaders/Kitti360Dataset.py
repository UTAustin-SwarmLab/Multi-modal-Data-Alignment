...
import glob
from mmda.liploc.utils.model_utils import get_transforms
from mmda.liploc.utils.rangeimage_utils import loadCloudFromBinary, createRangeImage
...
def get_filenames(sequences, data_path, length=None):
    ...
    for sequence in sorted(sequences):
        ...
        image_arr = np.arange(0, len(files)).astype(
            str
        )  # TODO seq 0002 does not start with 0000000000
...
def get_poses(eval_sequence, CFG):
    ...
    pose_file = (
        f"{CFG.data_path}/data_poses/2013_05_28_drive_{eval_sequence}_sync/poses.txt"
    )
...
class KITTI360Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        lidar_points = loadCloudFromBinary(
            f"{self.data_path}/data_3d_raw/2013_05_28_drive_{seq}_sync/velodyne_points/data/{instance}.bin"
        )  # seq ranges from 0000-0010

        if self.fisheye:
            left_fisheye_img = cv2.imread(
                f"{self.data_path}/data_2d_raw/2013_05_28_drive_{seq}_sync/image_02/data_rgb/{instance}.png"
            )
            right_fisheye_img = cv2.imread(
                f"{self.data_path}/data_2d_raw/2013_05_28_drive_{seq}_sync/image_03/data_rgb/{instance}.png"
            )
            equi_width = 1400
            equi_height = 1400
            ...
            if left_fisheye_img is None:
                ...
                left_fisheye_img = cv2.imread(
                    f"{self.data_path}/data_2d_raw/2013_05_28_drive_{seq}_sync/image_02/data_rgb/{str(int(instance) - 1).zfill(10)}.png"
                )
            ...
            if right_fisheye_img is None:
                ...
                right_fisheye_img = cv2.imread(
                    f"{self.data_path}/data_2d_raw/2013_05_28_drive_{seq}_sync/image_03/data_rgb/{str(int(instance) - 1).zfill(10)}.png"
                )

            left_equi_img = self.fisheye_to_equirectangular(
                left_fisheye_img, equi_width, equi_height
            )
            right_equi_img = self.fisheye_to_equirectangular(
                right_fisheye_img, equi_width, equi_height
            )

            image = np.concatenate((left_equi_img, right_equi_img), axis=1)
        else:
            image = cv2.imread(
                f"{self.data_path}/data_2d_raw/2013_05_28_drive_{seq}_sync/image_00/data_rect/{instance}.png"
            )
        ...
        lidar_image = createRangeImage(
            lidar_points, self.CFG.crop
        )  # TODO fix cropping for lidar range
    ...
    def fisheye_to_equirectangular(self, fisheye_img, output_width, output_height):
        ...
        output_width = (
            output_width * 2
        )  # output intermediate image needs to be twice in width
