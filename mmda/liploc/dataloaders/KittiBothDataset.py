...
from mmda.liploc.utils.rangeimage_utils import createRangeImage, loadCloudFromBinary
...
def get_poses(eval_sequence, CFG):
    ...
    if len(eval_sequence) == 2:
        ...
        poses = pd.read_csv(pose_file, header=None, sep="\s+").to_numpy()
...
class KITTIBothDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, transforms, CFG, filenames=[], sequences=[]):
        if len(filenames) != 0:
            ...
            self.data_filenames = get_filenames(
                sequences, CFG.data_path, CFG.data_path_360
            )
    ...
    def __getitem__(self, idx):
        ...
        if len(seq) == 2:
            ...
            image1 = cv2.imread(
                f"{self.data_path_360}/data_2d_raw/2013_05_28_drive_{seq}_sync/image_00/data_rect/{instance}.png"
            )
            lidar_points = loadCloudFromBinary(
                f"{self.data_path_360}/data_3d_raw/2013_05_28_drive_{seq}_sync/velodyne_points/data/{instance}.bin"
            )
