...
def get_caption(cfg: DictConfig) -> None:
    ...
    if cfg.dataset == "sop":
        path_text_descriptions = joblib.load(Path(cfg_dataset.paths.dataset_path + "text_descriptions_SOP.pkl"))
        for path_text in path_text_descriptions:
            path_text[0] = path_text[0].replace("/store/", "/nas/")
    elif cfg.dataset == "pitts":
        # val set
        val_path_text_descriptions = joblib.load("/nas/omama/datasets/pitts250k/text_descriptions_pitts30k.pkl")
    ...
    with Path(cfg_dataset.paths.dataset_path + f"{cfg.dataset}_{model_name}_captions.pkl").open("wb") as f:
