...
def main(cfg: DictConfig):  # noqa: D103
    ...
    roc_points = MusicCaps_align(cfg)
    ds_auc = cal_AUC(roc_points)
    roc_clap_points = MusicCaps_CLAP_align(cfg)
    ds_cos_auc = cal_AUC(roc_clap_points)
    # plot the ROC curve
    fig, ax = plt.subplots()
    # Ours
    ax.plot(
    ...
    # CLIP encoders
    ax.plot(
        [x[0] for x in roc_clap_points],
        [x[1] for x in roc_clap_points],
        "+-",
        label=f"Random shuffle (CLAP). AUC={ds_cos_auc:.3f}",
        color="blue",
    )
    # LLaVA
    ...
    if cfg.equal_weights:
        fig.savefig(cfg.paths.plots_path + f"ROC_curves_size{num_train_data}_dim{cfg.sim_dim}_noweight.png")
    else:
        fig.savefig(cfg.paths.plots_path + f"ROC_curves_size{num_train_data}_dim{cfg.sim_dim}.png")
    return


def MusicCaps_align(cfg: DictConfig) -> list[tuple[float, float]]:
    ...
    with open(cfg.paths.save_path + f"MusicCaps_text_emb_{cfg.text_encoder}.pkl", "rb") as f:
        Txt = pickle.load(f)
    print(f"Audio shape: {Audio.shape}, Text shape: {Txt.shape}")
    ...
    assert np.allclose(
        trainAudioAlign.mean(axis=0), 0, atol=1e-4
    ), f"trainAudioAlign not zero mean: {trainAudioAlign.mean(axis=0)}"
    assert np.allclose(
    ...
    trainAudioAlign, trainTxtAlign = audio_text_CCA.fit_transform((trainAudioAlign, trainTxtAlign))
    if cfg.equal_weights:
    ...
    valAudioAlign, valTxtAlign = audio_text_CCA.transform((valAudioAlign, valTxtAlign))
    ...
    # make sure the data is zero mean
    assert np.allclose(
        trainAudioUnalign.mean(axis=0), 0, atol=1e-4
    ), f"trainAudioUnalign not zero mean: {trainAudioUnalign.mean(axis=0)}"
    assert np.allclose(
        trainTxtUnalign.mean(axis=0), 0, atol=1e-4
    ), f"trainTxtUnalign not zero mean: {trainTxtUnalign.mean(axis=0)}"

    valAudioAlign, valTxtAlign = audio_text_CCA.transform((valAudioUnalign, valTxtUnalign))
    ...
    if cfg.equal_weights:
        save_fig(
            fig,
            cfg.paths.plots_path
            + f"similarity_score_dataset_r{cfg.train_test_ratio}_dim{cfg.sim_dim}_{cfg.train_test_ratio}_noweight.png",
        )
    else:
        save_fig(
            fig,
            cfg.paths.plots_path
            + f"similarity_score_dataset_r{cfg.train_test_ratio}_dim{cfg.sim_dim}_{cfg.train_test_ratio}.png",
        )

    # CCA dimensionality reduction
    audio_text_CCA_unalign = CCA(latent_dimensions=cfg.CCA_dim)
    trainAudioUnalign, trainTxtUnalign = audio_text_CCA_unalign.fit_transform((trainAudioUnalign, trainTxtUnalign))
...
def MusicCaps_CLAP_align(cfg):
    """Align the audio and text data and calculate the similarity score using CLAP method.
